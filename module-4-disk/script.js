/* ============================================================
   script.js — Module 4: Disk Scheduling
...
function addRequest() {
  ...
  if (!valid) {
    showError('⚠️ Invalid queue. Use comma-separated numbers within disk size range.');
    return;
  }
...
function runSimulation() {
  ...
  if (!rawQueue) { showError('⚠️ Please enter a request queue.'); return; }
  if (isNaN(diskSize) || diskSize < 2) { showError('⚠️ Disk size must be at least 2.'); return; }
  if (isNaN(headPos) || headPos < 0 || headPos >= diskSize) {
    showError(`⚠️ Head position must be between 0 and ${diskSize - 1}.`); return;
  }

  const parts = rawQueue.split(',').map(s => s.trim());
  if (!parts.every(p => /^\d+$/.test(p))) {
    showError('⚠️ Request queue must contain only comma-separated numbers.'); return;
  }

  const requests = parts.map(Number);
  if (requests.some(r => r < 0 || r >= diskSize)) {
    showError(`⚠️ All requests must be within disk range 0–${diskSize - 1}.`); return;
  }
  ...
  if (selectedAlgo === 'PDS') {
    ...
    if (!rawPri) { showError('⚠️ Please enter priorities for PDS.'); return; }
    const priParts = rawPri.split(',').map(s => s.trim());
    if (priParts.length !== parts.length) {
      showError(`⚠️ Number of priorities (${priParts.length}) must match number of requests (${parts.length}).`); return;
    }
    if (!priParts.every(p => /^\d+$/.test(p) && parseInt(p) >= 1)) {
      showError('⚠️ Priorities must be positive integers (1 = highest).'); return;
    }
